...
* gcd nsoperationqueue thread(区别)各有什么优势
...
这是查找能响应用户事件的顺序，当查找到能响应的控件后，就回按相反的顺序来查找能处理事件的响应者nextresponder，直到为空。

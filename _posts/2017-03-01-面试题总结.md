...
* NSString如何计算字符的个数？
[NSString是如何算长度的](https://objccn.io/issue-9-1/)
...
>用自己的话解释一下，有两个参数cwnd ssthresh，TCP建立了连接的时候 cwnd = 1然后TCP发送的包的数据大小也就是1，没收到1个ack cwnd*=2,直到cwnd>=ssthresh。这时候cwnd没过一个RTT开始呈线性增加+1。直到cwnd>=一个峰值 这时候cwnd还原为1 ssthresh也重新置为一个数ssthresh = cwnd / 2，依次循环。

[参考链接](https://blog.csdn.net/wdscq1234/article/details/52517420)
...
在header中加入--Connection:keep-alive在HTTP协议请求和相应中加入这条就能维持长连接。有效时间Keep-Alive: timeout=60
...
[参考链接](https://wereadteam.github.io/2016/02/22/MLeaksFinder/)

[2](http://www.cocoachina.com/ios/20170102/18490.html)

[3](https://cloud.tencent.com/developer/article/1004373)

[4](http://cnsue.me/2017/04/03/iOS%E5%86%85%E5%AD%98%E6%B3%84%E6%BC%8F%E8%87%AA%E5%8A%A8%E6%A3%80%E6%B5%8B/)
...
1. NSThread 可以先将自己的当前线程对象注册到某个全局的对象中去，这样相互之间就可以获取对方的线程对象，然后就可以使用下面的方法进行线程间的通信了，由于主线程比较特殊，所以框架直接提供了在主线程执行的方法
		...
		- (void)performSelectorOnMainThread:(SEL)aSelector withObject:(nullable id)arg waitUntileDone:(BOOL)wait;

		- (void)performSelector:(SEL)aSelector OnThread:(NSThread *)thr withObject:(nullable id)arg waitUntileDone:(BOOL)wait NS_AVAILABLE(10_5, 2_0);
...
3. NSOperation: 
		...
		[queue addOperationWithBlock:^{
			// 2.1在子线程中下载图片NSURL *url  = [NSURL URLWithString:@"http://imgcache.mysodao.com/s.JPG"];
		...
		NSBlockOperation *op1 = [NSBlockOperation blockOperationWithBlock:^{
			NSURL *url  = [NSURL URLWithString:@"x0x1.JPG"];
			NSData *data = [NSData dataWithContentsOfURL:url]; 
		...
		NSBlockOperation *op2 = [NSBlockOperation blockOperationWithBlock:^{
			NSURL *url  = [NSURL URLWithString:@"2365300.JPG"];
			NSData *data = [NSData dataWithContentsOfURL:url];		   image2 = [UIImage imageWithData:data];   
		 }];
		// 4.添加一个操作合成图片    
		NSBlockOperation *op3 = [NSBlockOperation blockOperationWithBlock:^{        	
			UIGraphicsBeginImageContext(CGSizeMake(200,200));        			[image1 drawInRect:CGRectMake(0,0,100,200)];	      	[image2 drawInRect:CGRectMake(100,0,100,200)];
			UIImage *res = UIGraphicsGetImageFromCurrentImageContext();        		UIGraphicsEndImageContext();// 5.回到主线程更新UI        			[[NSOperationQueue mainQueue] addOperationWithBlock:^{
...
hash函数(具体怎么实现自定义)根据key来确定一个hash地址,然后根据hash地址来找到数组地址。通常检索数组的最快办法是通过下标，但是如果想要查找数组中的某一个则要进行遍历，这样时间复杂度为O(n)，但是借助hash可以将时间复杂度将为O(1)。
...
###在iOS中的应用
如果判断两个对象是否是同一个对象直接用==，直接判断指针是否为同一个指针。
如果判断两个对象内容是否相同，则要自己实现-(void)isEqual方法。系统在调用这个方法前会先调用- (NSUInteger)hash算法判断是否相同。

hash值相等，对象不一定相等，对象相等，hash值一定相等。

[hash:简书](https://www.jianshu.com/p/2a71b027b723)

* ##7.NSDictionary的实现原理

主要针对的是- (void)setObject:(ObjectType)anObject forKey:(KeyType <NSCopying>)aKey;

首先通过key对象的hash函数算出key的hash地址，可以确定object在hash表内的入口位置，然后根据isEqual方法判断对象是否为相等，如果相等则说明NSDictionary中已经有这个key的值了。
...
可以看出来NSMapTable是一个哈希+链表的数据结构(马的怎么看出来的)，因此在NSMapTable中插入或者删除一对对象时

寻找到时间是O(1)+O(m),m最坏时可能为n。
...
> 首先setObject:ForKey:是NSMutableDictionary特有的；setValue:ForKey是KVC的主要方法。
> 1. setObject:ForKey:的value不能为nil，setValue:ForKey:是可以为nil的当为nil时，会调用removeObject:forKey方法。
> 2. setObject:ForKey:的key必须是不为nil的所有继承NSCopying协议的类型。setValue:ForKey的key必须是不为nil的字符串类型。

摘自[NSDictionary的内部结构和实现原理](https://www.jianshu.com/p/d4b5542740d5)

* ##8.你们的app是如何处理本地数据安全的(比如用户名和密码)

plist尽量不要存敏感信息，证件号地址等等(如果要存尽量加密后存，CommonCrypto)
sqlit也是不是加密数据库就是对数据库里的内容加密。
使用keychain
...
这里有空的话需要研究一下AFNetWorking的代码

[profile](http://lrd.ele.me/2017/02/09/%E9%80%A0%E8%BD%AE%E5%AD%90%20%7C%20%E5%A6%82%E4%BD%95%E8%AE%BE%E8%AE%A1%E4%B8%80%E4%B8%AA%E9%9D%A2%E5%90%91%E5%8D%8F%E8%AE%AE%E7%9A%84%20iOS%20%E7%BD%91%E7%BB%9C%E8%AF%B7%E6%B1%82%E5%BA%93/)
[profile](https://www.jianshu.com/p/d67f7d8bcc8a)
[profile](https://casatwy.com/iosying-yong-jia-gou-tan-wang-luo-ceng-she-ji-fang-an.html)
[profile](http://www.cocoachina.com/ios/20170104/18507.html)
...
[profile](https://www.cnblogs.com/DeasonGuan/articles/Hanami.html)

* ##25.设计一套缓存策略
* ##26.设计一个检测主线程卡顿的方案

[profile](https://cloud.tencent.com/developer/article/1030608)
[profile](https://www.jianshu.com/p/ea36e0f2e7ae)
[profile](https://juejin.im/entry/57faef5a67f356005878eb64)
[profile](https://blog.csdn.net/YCM1101743158/article/details/77508924)
